...
const WorkGrid: React.FC<WorkGridProps> = ({ limit, showAllButton = false, title = "Projetos", compact = false }) => {
  ...
  return (
    <section id="work" className={`bg-[#050505] px-4 md:px-8 ${compact ? 'py-8 md:py-12' : 'py-12 md:py-24'}`}>
      <div className="max-w-5xl mx-auto">
        <div className={`${compact ? 'mb-8' : 'mb-16'} text-center`}>
          <h3 className="text-4xl md:text-5xl font-bold text-white mb-8">{title}</h3>
          
          {/* Category Tabs */}
          <div className="flex flex-wrap justify-center gap-4">
            {(['Todos', 'Vídeo', 'Site', 'Design'] as Category[]).map((cat) => (
              <button
                key={cat}
          ...
          {displayedProjects.map((project) => (
            ...
            >
              <div className={`relative rounded-2xl overflow-hidden bg-[#111] mb-4 ${
                project.thumbnailOrientation === 'vertical' ? 'aspect-[4/5]' : 'aspect-video'
              }`}>
                <img 
              ...
              <div className="px-1">
                ...
                <p className="text-sm text-gray-500 mt-1">{project.category} • {project.year}</p>
              </div>
            </Link>
          ))}
        </div>
        ...
        {showAllButton && (
          <div className="mt-16 text-center">
            <Link to="/projects" className="inline-block px-8 py-3 bg-white text-black font-semibold rounded-full hover:bg-gray-200 transition-colors">
